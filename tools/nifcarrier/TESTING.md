...
3. `nifcarrier merge <tmp2>.nif <tmp>.nif <content2>.nif [...]`（ボーン和集合。
   ⚠️ 現行 merge は **shape を複製しない**＝add 側 content の XML が per-*-shape を参照する場合は
   その shape が欠ける（run4 の教訓）。add 側はコリジョン shape 無し XML の content にするか、
   shape 複製（CloneShape）を merge に増設してから使う）。
