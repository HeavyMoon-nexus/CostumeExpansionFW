...
- L1 で「動的フォームを捕獲したい」正当ユースを潰す可能性 — 現行アーキテクチャでは
  ...
  (r2 追記: allowDynamic はレビューにより撤去 — ハード不変条件化)

### 7.3-r6 フィールド実証された残余: worn ジュエリー捕獲 → MARA 内部 CTD(2026-07-25)

- TEST_RUN M4-J: 真正 v1.3.2 で、**装着中の MARA 管理アミュレット**を 2 点連続捕獲
  → 2 点目直後に **MARA.dll 内部で CTD**(`crash-2026-07-25-13-03-03.log`、
  全 8 フレーム MARA.dll・std::format 整形中・SKSE タスク起点。CEF ログ warn ゼロ)。
  §2 旧 H2(RemoveItem 剥ぎ取り再入)の実証 — 予測どおり **CEF 側の堅牢化では
  防げない**(MARA には正規 unequip でも落ちるバグ #1058804 があり、丁寧に
  unequip してから剥いでも安全保証なし)。
- **対象はジュエリー(slot35/36・ArmorJewelry)の worn 捕獲のみ**。ピッカー安全化・
  CORE Carrier 遮断・非ジュエリー捕獲・unworn ジュエリーのインベントリ捕獲
  (MARA は worn しか管理しない)は green。報告者返信 §7.1-r5 は「MARA オフ推奨」を
  含むため訂正不要。
- 対応オプション(オーナー判断):
  - **A: 文書化のみ** — README「Mod compatibility」MARA 節へ「MARA 稼働中は
    装着中のジュエリーを捕獲しない(外してから/インベントリから捕獲)」を追記し、
    v1.3.2 はこのまま出荷。
  - **B: ソフトガード追加** — 捕獲ゲート(`CanCaptureContent` 限定・登録側は不変)に
