...
namespace CostumeFW::poolspike
{
    namespace
    {
        ...
        void LoadRegistry()
        {
            ...
            try {
                ...
                SKSE::log::error("poolspike: registry unreadable ({}) - no slots restored", e.what());
                return;
            }
        ...
        void SaveRegistry()
        {
            ...
            if (!out) {
                SKSE::log::error("poolspike: could not write {}", kRegistryPath);
                return;
            }
            out << doc.dump(2);
        }
        ...
        bool EnsureOff(RE::SpellItem* a_spell, const char* a_why)
        {
            ...
            if (player->HasSpell(a_spell)) {
                SKSE::log::error("poolspike: RemoveSpell did NOT take {:08X} off ({}) - refusing to "
        ...
        bool Hydrate(int a_slot, const char* a_why)
        {
            ...
            if (!spell) {
                SKSE::log::error("poolspike: slot {} - {} does not resolve. Is {} enabled?", a_slot,
            ...
            if (!ench) {
                SKSE::log::error("poolspike: slot {} source '{}' is not a resolvable enchantment",
                    a_slot, rec->source);
                return false;
            }
            ...
            SKSE::log::info("poolspike: slot {} ({:08X}) hydrated {} [{}] - {} effect(s), {} "
                            "conditioned, from {}",
    ...
    void HydrateAtDataLoaded()
    {
        ...
        if (early || late || failed) {
            ...
            SKSE::log::info("poolspike: kDataLoaded - {} slot(s) hydrated early, {} left for the "
                            "console (late), {} failed",
                early, late, failed);
        }
    }
    ...
    void PoolCommand(RE::TESObjectREFR*, const std::string& a_args)
    {
        ...
        if (sub.empty() || sub == "list") {
            LoadRegistry();
            Print(std::format("[CEF poolspike] {} slots ({})", kPoolSize, kPoolPlugin));
            for (int i = 0; i < kPoolSize; ++i) {
                Print(Describe(i));
                SKSE::log::info("poolspike:{}", Describe(i));
            }
            return;
        }
        ...
        if (sub == "clear") {
            ...
            g_slots = {};
            SaveRegistry();
            Print("[CEF poolspike] every slot released and the registry emptied");
            return;
        }
        ...
        if (slot < 0 || slot >= kPoolSize) {
            Print(std::format("[CEF poolspike] usage: cef poolspike [list | clear] | "
                              "cef poolspike <set|on|off|hydrate> <0-{}> [args]",
                kPoolSize - 1));
            return;
        }
        ...
        if (sub == "set") {
            ...
            if (source.find(':') == std::string::npos) {
                Print("[CEF poolspike] usage: cef poolspike set <slot> <FormID:Plugin.esp of an ENCH> [late]");
                return;
            }
            Recipe r;
            r.source = source;
            ...
            SaveRegistry();
            if (Hydrate(slot, "set")) {
                Print(std::format("[CEF poolspike] slot {} = {} ({}) - hydrated now and saved to the "
                                  "registry",
                    slot, source, r.early ? "early" : "late"));
                Print(Describe(slot));
            } else {
                Print(std::format("[CEF poolspike] slot {} recorded, but hydration FAILED - see the log",
                    slot));
            }
            return;
        }
        ...
        if (sub == "hydrate") {
            Print(Hydrate(slot, "console")
                      ? std::format("[CEF poolspike] slot {} refilled", slot)
                      : std::format("[CEF poolspike] slot {} could not be refilled - see the log", slot));
            return;
        }
        ...
        auto* player = RE::PlayerCharacter::GetSingleton();
        if (!spell || !player) {
            Print(std::format("[CEF poolspike] slot {} does not resolve (is {} enabled?)", slot,
                kPoolPlugin));
            return;
        }
        ...
        if (sub == "on") {
            if (spell->effects.empty()) {
                Print(std::format("[CEF poolspike] slot {} is empty - `cef poolspike set` it first. An empty "
                                  "ability grants nothing and measures nothing",
                    slot));
                return;
            }
            if (player->HasSpell(spell)) {
                Print(std::format("[CEF poolspike] slot {} is already granted", slot));
                return;
            }
            ...
            if (!player->HasSpell(spell)) {
                Print(std::format("[CEF poolspike] slot {} did NOT go on - AddSpell was refused", slot));
                SKSE::log::error("poolspike: AddSpell did NOT take slot {} ({:08X})", slot,
                    spell->GetFormID());
                return;
            }
            SKSE::log::info("poolspike: granted slot {} ({:08X}), {} effect(s)", slot,
            ...
            Print(std::format("[CEF poolspike] slot {} granted - wait ~10s before `cef av`, the "
        ...
        if (sub == "off") {
            if (!player->HasSpell(spell)) {
                Print(std::format("[CEF poolspike] slot {} was not granted", slot));
                return;
            }
            player->RemoveSpell(spell);
            if (player->HasSpell(spell)) {
                Print(std::format("[CEF poolspike] slot {} REFUSED to come off - that is a result, "
                                  "write it down",
                    slot));
                SKSE::log::error("poolspike: RemoveSpell did NOT take slot {} ({:08X}) off", slot,
                    spell->GetFormID());
            } else {
                Print(std::format("[CEF poolspike] slot {} removed - now run `cef av`", slot));
                SKSE::log::info("poolspike: removed slot {} ({:08X})", slot, spell->GetFormID());
            }
            return;
        }

        Print("[CEF poolspike] list | set <slot> <ENCH id> [late] | on <slot> | off <slot> | "
              "hydrate <slot> | clear");
    }
}
