...
| C7 動的フォーム/セーブ | 0xFF 流入・id 破損 | **潜在バグあり** | 同左 | 同左 | **L1+L2+buf 修正+native ゲートで閉じる** |

**開いていた面は C1 と C7 のみ**。C7 は v1.3.2 で完全に閉じる。C1 は「CEF だけがロードする
content の不正 NIF」という残余が残る(§4.1 — BSA 内 NIF は事前検証不能のため、v1.3.2 では
流入経路の遮断=L1 と既存ゲート維持に留め、誠実に限界を記す)。
...
2. **[B] 死体再装備**: publish トークン装備 NPC を殺害→セル再訪 — クラッシュしないこと
   (ゲート追加は beta 次版)。
3. **v1.3.2 ガード**: MARA 導入環境で `+ Add worn item` を開き CORE Carrier が**出ない**こと。
   `allowDynamic:true`+`disableDefaults:true` で出す→選択 = 従来 CTD の再現(ログ収集環境でのみ)。
