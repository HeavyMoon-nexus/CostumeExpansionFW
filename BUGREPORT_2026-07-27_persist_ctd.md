...
SoftBody 3.08 は **`skeleton.nif` / `skeletonbeast.nif` を上書き**し、
**SMP XML を 29 本**持ち込む。CEF の persist キャリアは全 persist コンテンツの
ボーンを 1 つの head-part にマージするので、単体で 1000 本規模になる
(実測 1113 + 1307)。SoftBody のボディ衝突ボーンがそこに乗ると、
**FSMP / エンジンのボーン上限に当たって CEF 側の分がマージされない**、
という筋が最も素直。報告者が **Bone Limit Extender** を要求されているのも符合する。
