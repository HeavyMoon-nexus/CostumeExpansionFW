...
**CEF 単体で 2226 + 1227 = 約 3450 ボーン**を要求している。これは
Bone Limit Extender で上限を上げてもなお他 mod と共存しづらい規模であり、
**SMP を多用する mod が同居した瞬間に、落とされるのは CEF の側**になる。

> **訂正(オーナー指摘):** Bone Limit Extender は CEF のサブ要件として
> **ローカルにも以前から導入済み**。よって「BLE が無いから」ではない。
> **BLE で引き上げた上限すら、SoftBody + CEF で超えている**という読みになる。
